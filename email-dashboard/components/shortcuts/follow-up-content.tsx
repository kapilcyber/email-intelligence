          ...
          <div className="max-w-full overflow-x-auto pb-1 [-webkit-overflow-scrolling:touch] sm:overflow-visible sm:pb-0">
            <div className="grid min-w-[18rem] grid-cols-7 gap-0.5 text-center text-[10px] sm:min-w-0 sm:gap-1 sm:text-xs">
              {p.days.map((day: FollowUpTrackerDay) => {
                ...
                const inPersonalSendWindow = personalDeadlineIso != null && dIso < personalDeadlineIso;
...
export function FollowUpContent({ embedded = false }: { embedded?: boolean }) {
      ...
      >
        <div className="min-w-0 flex-1">
          ...
          {weekHint && (
            <p
              className={cn(
                "break-words text-xs leading-relaxed text-neutral-500 dark:text-neutral-400",
                !embedded && "mt-1 sm:mt-2"
              )}
            >
              {weekHint}
            </p>
