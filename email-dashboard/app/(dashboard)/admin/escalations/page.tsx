...
export default function AdminEscalationsPage() {
  ...
  // Refetch user list when tab gains focus (when on user list view)
  useEffect(() => {
    if (status !== "authenticated" || selectedUserEmail) return;
    const onFocus = () => {
      loadUserCounts();
  ...
  }, [status, api, selectedUserEmail, page, pageSize, teamFilter, fromDate, toDate]);

  useEffect(() => {
    if (selectedUserEmail) loadCharts();
  }, [status, api, selectedUserEmail, fromDate, toDate, teamFilter]);
  ...
  const filteredItems = useMemo(() => {
    ...
    if (priorityFilter) {
      list = list.filter((i) => (i.priorityLabel ?? "") === priorityFilter);
    }
    return list;
      ...
      {!selectedUserEmail && (
        <section className="min-w-0 overflow-hidden rounded-3xl border border-indigo-100 bg-gradient-to-br from-white via-indigo-50/80 to-violet-50/90 p-4 shadow-md shadow-indigo-100/50 dark:border-neutral-700 dark:from-neutral-900 dark:via-neutral-900 dark:to-neutral-950 dark:shadow-none sm:p-6">
          <CardHeader className="p-0 pb-3 sm:pb-4">
            ...
            <p className="mt-1 text-xs text-neutral-500 dark:text-neutral-400 md:hidden">Tap a user to open their mailbox escalations.</p>
          </CardHeader>
                    ...
                    <li key={u.email}>
                      ...
                      >
                        ...
                        <div className="flex items-center justify-between gap-2">
                          <span className="rounded-full bg-amber-100 px-2 py-0.5 text-xs font-medium tabular-nums text-amber-800 dark:bg-amber-900/40 dark:text-amber-300">
                            {u.escalationCount} escalations
                          </span>
                      ...
                      <tbody className="divide-y divide-neutral-200 dark:divide-neutral-700">
                        {userCounts.map((u) => (
                          ...
                          >
                            ...
                            <td className="px-4 py-3">
                              <span className="text-xs text-blue-600 dark:text-blue-400">View escalations →</span>
                            </td>
                          </tr>
                        ))}
                      </tbody>
            ...
            <section className="min-w-0 rounded-3xl border border-orange-100 bg-gradient-to-br from-white via-orange-50 to-amber-50 p-4 shadow-md shadow-orange-100/60 dark:border-neutral-800 dark:from-neutral-900 dark:via-neutral-900 dark:to-neutral-950 dark:shadow-none sm:p-6">
              ...
              {chartLoading ? (
                ...
                <div className="h-[220px] w-full min-w-0 sm:h-[280px] [&_.recharts-cartesian-axis-tick_text]:fill-slate-500 dark:[&_.recharts-cartesian-axis-tick_text]:fill-neutral-400">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={barData} margin={{ top: 16, right: 8, left: -12, bottom: 4 }} barCategoryGap="18%">
                      <defs>
                        {barData.map((d, i) => (
